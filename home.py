...
if application_scenario == "Steel Plate Stacking":
    ...
    if df is not None:
        ...
        class GA_with_Batch:
            ...
            def __init__(self, population_size, mutation_rate, crossover_rate, generations, lambda_1, lambda_2,
                         lambda_3,
                ...
                self.lambda_4 = lambda_4
                self.num_positions = num_positions
                self.population = [np.random.randint(0, num_positions, size=len(plates)) for _ in
                    ...
                    for generation in range(self.generations):
                        ...
                        if self.use_adaptive:
                            self.mutation_rate, self.crossover_rate = apply_adaptive_ga(
                                self.mutation_rate, self.crossover_rate, best_score_gen, self.best_score,
                                self.use_adaptive
                            )
                            self.record_adaptive_params()
            ...
            def update_convergence_plot(self, current_generation):
                ...
                # 保存收敛图
                save_convergence_plot(self.convergence_data, current_generation, self.best_score, "GA",
                                      self.dataset_name)
        ...
        class PSO_SA_Optimizer:
            ...
            def get_stable_iterations(self):
                ...
                for i in range(1, len(self.convergence_data_pso_sa)):
                    if abs(self.convergence_data_pso_sa[i][1] - self.convergence_data_pso_sa[i - 1][
                        1]) < stable_threshold:
                        stable_iterations += 1
                return stable_iterations
        ...
        class MOEAD_with_Batch:
            ...
            def update(self, population, fitness_values, weights, neighbors):
                ...
                for i, individual in enumerate(population):
                    ...
                    for j in neighbors[i]:
                        if self.tchebycheff(child_fitness, weights[j]) < self.tchebycheff(fitness_values[j],
                                                                                          weights[j]):
                            population[j] = child
            ...
            def optimize(self):
                ...
                with st.spinner("Running MOEA/D Optimization..."):  # 提供运行时加载提示
                    ...
                    # 迭代优化
                    for generation in range(self.generations):
                        self.population = self.update(self.population, self.fitness_values, self.weights,
                                                      self.neighbors)
